     ...
     <div class="container">
         ...
         if ($result2->num_rows > 0) {
             while ($row2 = $result2->fetch_assoc()) {
                 ...
                 // Consulta para ver si hay sitios en esta sección
                 $sqlSitios = "SELECT * FROM sitios WHERE seccion='$sexi' ORDER BY id DESC";
                 $resultSitios = $conn->query($sqlSitios);

                 if ($resultSitios->num_rows > 0) {
                     // Solo muestra la sección si hay sitios
                     if ($koso === 0) {
                         echo "<h3 style='margin-top: 25px'>" . $row2['nombre'] . "</h3>";
                     } else {
                     ...
                     $cuenta = 1;
                     while ($row = $resultSitios->fetch_assoc()) {
                         if ($cuenta === 5) {
                             $cuenta = 1;
                         }
                         if ($cuenta === 1) {
